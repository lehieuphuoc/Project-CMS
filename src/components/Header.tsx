...
import { Input, Drawer, Divider, Modal } from "antd";
import {
    UserOutlined,
    SearchOutlined,
    HomeOutlined,
...
export default function Header() {
    ...
    // Theo dõi kích thước màn hình để biết đang ở mobile hay desktop
    useEffect(() => {
    ...
    return (
        <header className="sticky top-0 z-50 bg-white border-b border-gray-200 shadow-sm">
            ...
            <nav className="hidden md:flex justify-center items-center px-4 md:px-10 py-3 bg-white text-[15px] font-semibold text-gray-700 border-t border-gray-100">
                <div className="flex items-center gap-6 overflow-x-auto whitespace-nowrap scrollbar-hide">
                    ...
                    {menuItems.map((item, i) => (
                        ...
                        >
                            {item}
                            <span className="absolute left-0 bottom-[-4px] w-0 h-[2px] bg-green-600 transition-all duration-300 group-hover:w-full"></span>
                        </Link>
                    ))}
                </div>
            </nav>
            ...
            <Drawer
                title={
                    <div className="flex justify-between items-center w-full">
                        <Image src="/logo-pc.png" alt="Logo" width={90} height={35} />
                        <CloseOutlined
            ...
            {/* --- Overlay Search (Desktop / Mobile) --- */}
            {isMobile ? (
                // Giao diện mobile — full màn hình
                <Drawer
                    placement="top"
                    closable={false}
                    height="100%"
                ...
                // Giao diện desktop — Modal giữa màn hình
                <Modal
                    open={searchVisible}
                    footer={null}
                    closable={false}
